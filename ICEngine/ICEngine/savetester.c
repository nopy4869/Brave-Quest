...
{
	//Yes, all of this just to hijack the main function.... so sue me
	initscr();
	scrollok(stdscr, TRUE);
	clearsavemem();
	self.face = 1;
	...
	self.x = 76;
	self.y = 42;
	currgame.characters[0].name[0] = 'T';
	currgame.characters[0].luck = 65535;
	currgame.characters[1].name[0] = 'a';
	currgame.characters[1].exp = 31;
	currgame.characters[4].name[0] = 'Q';
	currgame.characters[4].speed = 53;
	savegame();
	clearsavemem();
	loadsave();
//	wprintw(stdscr, "%d", currgame.characters[0].luck);
	bie();
	return 0;
}
